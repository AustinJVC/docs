...
```
POST https://theslow.net/api/slowstats/ingest-metric
```
...
```bash
curl -X POST "https://theslow.net/api/slowstats/ingest-metric"      -H "Content-Type: application/json"      -H "X-API-KEY: YOUR_API_KEY_HERE"      -d '{
       "projectId": "abc123",
       "payload": {
         "temperature": 72.5,
         "humidity": 45
...
response = requests.post("https://theslow.net/api/slowstats/ingest-metric", json=data, headers=headers)
print(response.json())
```
...
```javascript
fetch("https://theslow.net/api/slowstats/ingest-metric", {
  method: "POST",
  headers: {
